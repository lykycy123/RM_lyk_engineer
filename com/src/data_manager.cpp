...
DataManager::DataManager(SendData *new_data) : data_(new_data)
{ //不如就使用pimpl手法!,注意shared_ptr构造函数只接受非const
    data_->start_flag = 's';
...
int DataManager::get_error(const int &Number_yellow, const vector<cv::Point> &target_points)
{
    int error_x = 0;
    if (Number_yellow == 1)
        error_x = target_points[0].x - g_para.src_width_ / 2 + g_para.offset;
    else if (Number_yellow == 3)
        error_x = minPoint(target_points[0], target_points[1], target_points[2]).x - g_para.src_width_ / 2;
    else if (Number_yellow == 2)
        error_x = minPoint(target_points[0], target_points[1]).x - g_para.src_width_ / 2;
