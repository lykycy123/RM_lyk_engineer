...
void Mineral::getSendData(SendData &data, int index = 1)
{
    ...
    int error_x = 0;
    if (Number_yellow == 1)
        error_x = target_points[0].x - src.size().width / 2;
    else if (Number_yellow == 2)
        error_x = minPoint(target_points[0], target_points[1]).x - src.size().width / 2;
    ...
    if (error_x < 0)
    {
        ...
        if (data.error_x > threshold_error)
            data.error_x = threshold_error;
    }
    else
    {
        data.direction_x = 'r';
...
void Mineral::getSendData(SendData &data)
{
    ...
    { //有两个特征点，正常情况
        ...
        if (error_x < 0)
        {
            data.direction_x = 'l';
            data.error_x = 0 - error_x;
            if (data.error_x > threshold)
                data.error_x = threshold; //输出限幅
        }
        else
        {
            data.direction_x = 'r';
